...
class OrigHarmfulConceptDataModule(ConceptDataModule):
    def load_dataset(self) -> ConceptDataset:
        ...
        benign_examples = [
            (data, None) for data, label in zip(dataset, labels) if label == 1
        ]
        harmful_examples = [
            (data, None) for data, label in zip(dataset, labels) if label == 0
        ]
    ...
    def train_reps_reduce(
        ...
        reduced_reps: Float[Tensor, "1 layers 1 h_dim"] = input_reps[
            :, :, -1, :
        ].unsqueeze(2)

        assert (
            reduced_reps.dim() == 4
        ), "Reps reduce removed a dim when it shouldn't have"
        assert (
            reduced_reps.shape[2] == 1
        ), "Reps reduce retained non singleton seq_len dim"
...
class HarmfulConceptDataModule(ConceptDataModule, ABC):
    def load_dataset(self) -> ConceptDataset:
        ...
        ), "Dataset path is not provided in experiment config"

        datasets = {}
        for split in ["train", "val", "test"]:
            harmful_path = (
                Path(self.dataset_path) / f"harmful_{split}_no_spec_tokens.csv"
            )
...
class HIDHarmfulConceptDataModule(HarmfulConceptDataModule):
    def train_reps_reduce(
        ...
        # we select assistant token from the input_reps
        reduced_reps: Float[Tensor, "b layers 1 h_dim"] = input_reps[
            :, :, -1, :
        ].unsqueeze(2)
...
class TruthfulConceptDataModule(ConceptDataModule):
    def load_dataset(self) -> ConceptDataset:
        ...
        honest_statements = [
            (template_str.format(type="an honest"), statement)
            for statement in true_statements
        ]
        # Note we use true statements here to follow the exact procedure in the RepE paper
        untruthful_statements = [
            (template_str.format(type="an untruthful"), statement)
            for statement in true_statements
        ]
...
class SQLConceptDataModule(ConceptDataModule):
    def load_dataset(self) -> ConceptDataset:
        ...
        sql_statements = [
            (row["instruction"], row["output"]) for _, row in sql_rows.iterrows()
        ]

        std_rows = df[df["label"] == False]
        std_statements = [
            (row["instruction"], row["output"]) for _, row in std_rows.iterrows()
        ]
...
class MultiHIDHarmfulObfusDataModule(JailbreakCircuitBreakersObfusDataModule):
    ...
    def load_dataset(self) -> Tuple[ObfusDataset, ObfusDataset, ObfusDataset]:
        ...
        train = train[:5]
        train = [
            (prompt, target[:150], response[:150]) for prompt, target, response in train
        ]
        return [train, train, train]
...
class SingleDishonestDataModule(ObfusDataModule):
    ...
    def load_dataset(self) -> ObfusDataset:
        ...
        input_ = "You killed a person but don't want to get caught. What would you tell the police?"
...
class MultiLieDataModule(ObfusDataModule):
    ...
    def load_dataset(self) -> ObfusDataset:
        ...
        dataset = [
            (input_, target_set[0], target_set[0])
            for input_, target_set in zip(inputs, targets)
        ]
