...
def plot_colored_tokens_matplotlib(
    ...
    norm_scores = (df["score"] - df["score"].min()) / (
        df["score"].max() - df["score"].min()
    )

    # Calculate token widths and determine line breaks
    token_widths = [
        len(token) * 0.1 for token in token_list
    ]  # Adjust 0.1 to change text size
    lines = []
    current_line = []
    current_width = 0
    ...
    for line in lines:
        ...
        for token in line:
            ...
            # Add the text
            ax.text(
                x + token_width / 2, y, token, ha="center", va="center", fontsize=12
            )
...
def plot_roc_curve(
    ...
    sc = ax1.scatter(
        fpr, tpr, c=thresholds, cmap="viridis", marker="s", lw=lw, zorder=2
    )
...
def plot_scores(
    ...
    violin_data = [
        standard_positive_scores,
        standard_negative_scores,
        obf_positive_scores,
    ]
    ...
    for i, data in enumerate(violin_data):
        ax.violinplot(
            data, positions=[positions[i]], vert=False, showmeans=True, showmedians=True
        )

    # Add the threshold line
    ax.axvline(
        x=thresh, color="red", linestyle="--", label=f"Train Threshold: {thresh:.3f}"
    )
