...
class ContinuousGradientOptimizer(OptimizerBase, ABC):
    ...
    def step(self, batch) -> None:
        ...
        rep_output = self.model.forward_from_string(
            input_text=input_, target_text=rep_source
        )
        ...
        with torch.no_grad():
            ...
            torch.nn.utils.clip_grad_norm_(
                self.model.tunable_params.params, max_norm=1.0
            )
...
class AdamOptimizer(ContinuousGradientOptimizer):
    ...
    def init_optimizer(self, model: ModelBase, lr: float) -> torch.optim.Optimizer:
        # TODO: Hardcoding eps to work for half precision models
        return Adam([model.tunable_params.params], lr=lr, eps=1e-5)


class ConstraintedAdamOptimizer(ContinuousGradientOptimizer):

    def init_optimizer(self, model: ModelBase, lr: float) -> torch.optim.Optimizer:
        return ConstrainedAdam(
            [model.tunable_params.params],
            [model.tunable_params.params],
            lr=lr,
            eps=1e-5,
        )
...
class SignedContinuousOptimizer(OptimizerBase):
...
class GCGOptimizer(OptimizerBase):
    ...
    def step(self, batch) -> None:
        ...
        def get_ids_and_mask(input_str, **kwargs):
            return (
                tokenizer(input_str, return_tensors="pt", **kwargs)["input_ids"].to(
                    device
                ),
                tokenizer(input_str, return_tensors="pt", **kwargs)[
                    "attention_mask"
                ].to(device),
            )

        optim_ids, optim_attn_mask = get_ids_and_mask(
        ...
        for i in tqdm(range(num_steps)):
            ...
            behavior_output = self.model.forward_from_embeds(
                ...
                past_key_values=self.prefix_cache if self.use_prefix_cache else None,
            )
            loss: Float[Tensor, "b"] = self.loss_fn.compute_loss(
                behavior_logits=behavior_output.target_logits,
            ...
            if self.use_prefix_cache:
                ...
                input_attn_mask = torch.cat(
                    [
                        torch.ones(
                            new_search_width, sampled_top_embeds.shape[1], device=device
                        ),
            ...
            self.model.tunable_params.params = F.one_hot(
                optim_ids, embed_layer.num_embeddings
            ).to(dtype=embed_layer.weight.dtype, device=embed_layer.weight.device)
    ...
    def compute_candidates_loss(
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                target_attn_mask_batch = target_attn_mask.repeat(
                    input_embeds_batch.shape[0], 1
                )
            ...
            outputs = self.model.forward_from_embeds(
                ...
                past_key_values=prefix_cache_batch if self.use_prefix_cache else None,
            )
...
class FLRTOptimizer(OptimizerBase):
    ...
    def step(self, batch):
        ...
        buffer = AttackBuffer(
            model, init_len=self.config.init_len, size=self.config.buffer_size
        )
        ...
        for i in tqdm(range(self.config.num_steps)):
            ...
            # TODO: Fix filtering
            # print(f'new_attack_ids shape: {new_attack_ids.shape}')
            # new_ids = new_attack_ids
            # new_ids = filter_ids(new_attack_ids, model.tokenizer)
            ...
            # buffer.replace_best(optim_id, optim_loss, optim_monitor_loss, optim_generator_loss)
            ...
            # self.logger.log({"monitor_loss": buffer.monitor_losses.min().item()})
            # self.logger.log({"generator_loss": buffer.generator_losses.min().item()})
            opstr = optim_str.replace("\n", r"\n")
            print(f"Step {i}: Optim String = {opstr}")
            monitor_condition = (
                optim_monitor_loss < torch.tensor(0.5, device=model.device).item()
            )

            # if monitor_losses[-1] < 1e-3:
            #    skip_cond = True
            #    print("Skipping because monitor loss < 1e-3")
            # if self.generator_weight == 0:
            #    if monitor_condition:
            #        print("Early Stopping Condition Met")
            #        early_stopping_condition.append(1)
            #    else:
            #        early_stopping_condition.append(0)
            # elif self.monitor_weight == 0:
            #    if forcing_condition:
            #        print("Early Stopping Condition Met")
            #        early_stopping_condition.append(1)
            #    else:
            #        early_stopping_condition.append(0)
            # elif self.monitor_weight > 0 and self.generator_weight > 0:
            #    if forcing_condition and monitor_condition:
            #        print("Early Stopping Condition Met")
            #        early_stopping_condition.append(1)
            #    else:
            #        early_stopping_condition.append(0)

        # TODO: Update the self.model.tunable_params with the best optim_id
    ...
    def sample_candidates(
        ...
        special_ids = [
            ...
            1,
            2,
        ]  # TODO: hardcoded from tokenizer.all_special_ids for now
        ...
        for i in range(0, input_embeds.shape[0], batch_size):
            with torch.no_grad():
                ...
                if kv_cache is not None and not isinstance(kv_cache, HybridCache):
                    ...
                    reps: Float[Tensor, "b_size layers seq_len hidden_size"] = (
                    ...
                    logits = outputs.logits[..., before_embeds.shape[1] :, :]
                    # hidden_states = [
                    #    hs[..., before_embeds.shape[1] :, :] for hs in outputs.hidden_states
                    # ]
                    reps: Float[Tensor, "b_size layers seq_len hidden_size"] = (
                        torch.stack(outputs.hidden_states, dim=1)
                    )
                ...
                forcing_condition = torch.all(
                    torch.eq(model_preds, shift_labels), dim=-1
                )
                ...
                # Insert loss clamping here
                # generator_loss = torch.nn.functional.cross_entropy(
                #    shift_logits.view(-1, shift_logits.size(-1)),
                #    shift_labels.view(-1),
                #    reduction="none",
                # )
                # clamp loss to -ln(0.6)
                # generator_loss = torch.clamp(
                #    generator_loss, min=-torch.log(torch.tensor(0.6, device=self.model.device))
                # )
                # generator_loss = generator_loss.view(current_batch_size, -1).mean(dim=-1)
                # self.clear_gpus()

                # loss = generator_weight * generator_loss
                all_loss.append(loss)
                all_generator_loss.append(loss)
                all_monitor_loss.append(loss)  # TODO: Implement monitor loss
