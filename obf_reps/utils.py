...
from obf_reps.optimize.loss import (
    ...
    PinkElephantPatternLoss,
    ProbePredictLoss,
)
from obf_reps.optimize.optimizers import (
    AdamOptimizer,
    ConstraintedAdamOptimizer,
    FLRTOptimizer,
    GCGOptimizer,
    OptimizerConfig,
)
...
def validate_and_create_optimizer_config(
    ...
    assert isinstance(
        seed, int
    ), f"seed must be an int, got {seed} of type {type(seed)}"
...
# TODO: This can probably be done WAY cleaner with Pydantic
def validate_and_create_experiment_config(cfg: DictConfig) -> ExperimentConfig:
    ...
    if (
        cfg.obfus_data_path is not None
        and not pathlib.Path(cfg.obfus_data_path).exists()
    ):
    ...
    match cfg.optimizer:
        ...
        case "ConstraintedAdamOptimizer":
            optimizer_cls = ConstraintedAdamOptimizer
        case "FLRTOptimizer":
    ...
    # Validate seed
    # if getattr(cfg, "seed", None) is None:
    #     seed = 0  # default to 0
    # else:
    ...
    match cfg.loss:
        ...
        case "PinkElephantPatternLoss":
            loss_cls = PinkElephantPatternLoss
        case _:
    ...
    match cfg.concept_data_module:
        ...
        case _:
            raise ValueError(
                f"concept_data_module {cfg.concept_data_module} not supported"
            )
    ...
    match cfg.logger:
        ...
        case "WAndBLogger":
            ...
            logger = WAndBLogger(
                log_file=cfg.log_file, username=cfg.username, metadata=dict_cfg
            )
        case _:
    ...
    if load_tunable_params_path_direct is not None:
        ...
        load_tunable_params_path = (
            load_tunable_params_path
            / cfg.concept_data_module
            / f"model-params-{cfg.metric}.pkl"
        )
    ...
    if load_probe_path is not None:
        ...
        load_probe_path = (
            load_probe_path / cfg.concept_data_module / f"{cfg.metric}.pkl"
        )
    save_probe_path = cfg.get("save_probe_path", None)
    if save_probe_path is not None:
        save_probe_path = Path(save_probe_path)
        save_probe_path = (
            save_probe_path / cfg.concept_data_module / f"{cfg.metric}.pkl"
        )
        ...
        if path is not None:
            ...
            if path_name.startswith("load_"):
                ...
                if not path_obj.parent.exists():
                    ...
                    print(
                        f"Created directory structure for {path_name}: {path_obj.parent}"
                    )
                if path_obj.is_dir():
    ...
    concept_data_module = concept_data_module_cls(
        # TODO: Might wanna split batch size into separate batch_sizes for obfus and concept
...
def gen_return_to_LAT_scans(
    ...
    for gen_return, score in zip(gen_returns, scores):
        ...
        gen_token_text = [
            [model.tokenizer.decode(x) for x in gen_return.gen_ids.squeeze()]
        ]
