...
class HFModelBase(ModelBase, ABC):
    ...
    def tokenize(
        ...
        if pad_to_max_length:
            ...
            ), "max_length must be set when using pad_to_max_lenght"
    ...
    def forward_from_string(
        ...
        input_ids, input_attn_mask = self.tokenize(
            input_text, add_chat_template=add_chat_template
        )
...
class HFModelPrompted(HFModelBase, ABC):
    ...
    def __init__(
        ...
        self.tokenizer.add_special_tokens(
            {"additional_special_tokens": [self.OPT_LOC_TOKEN]}
        )
    ...
    def forward_from_embeds(
        ...
        layers_to_probe: Optional[
            List[int]
        ] = None,  # TODO: consider making this a variable in HFModelPromptedWithSelectableLayers init, rather than arg here
    ) -> ForwardReturn:
        ...
        if (
            ...
            input_loss_mask = (
                torch.ones(
                    batch_size, input_seq_len, dtype=torch.bool, device=self.device
                )
                * -100
            )
    ...
    def generate_from_ids(
        ...
        **generate_kwargs,
        ...
        # Get text and reps
        gen_text: List[str] = self.tokenizer.batch_decode(
            gen_ids, skip_special_tokens=False
        )
        input_text: List[str] = self.tokenizer.batch_decode(
        ...
        input_reps_list: List[Float[Tensor, "b_size input_seq_len hidden_size"]] = (
            hidden_states[0]
        )
        input_reps: Float[Tensor, "b_size layers input_seq_len hidden_size"] = (
            torch.stack(input_reps_list, dim=1)
    ...
    def _convert_ids_to_input_embeds(
        ...
        if not torch.any(input_ids == self.opt_loc_token_id):
            ...
            left_input_ids = input_ids[
                :, :insertion_column
            ]  # All columns before the N column
            ...
            if use_tunable_params:
                ...
                batched_param_embeds: Float[Tensor, "b seq_len hidden_size"] = (
                    param_embeds.expand(batch_size, -1, -1)
                )
                ...
                attention_mask = torch.cat(
                    [left_attention_mask, right_attention_mask], dim=1
                )
...
class HFHardPromptedWithSelectableLayers(HFHardPrompted):
    ...
    def forward_from_embeds(
        ...
        #
        if target_ids is not None and target_ids.numel() > 0:
            ...
            input_loss_mask = (
                torch.ones(b_size, input_seq_len, dtype=torch.bool, device=self.device)
                * -100
            )
            target_loss_mask = target_ids.clone()
            ...
            for L in layer_indices:
                ...
                input_rep_list.append(
                    inp.unsqueeze(1)
                )  # => [b_size, 1, seq_len, hidden_dim]
            ...
            for L in layer_indices:
                all_tokens = collected_hidden[L]
                input_rep_list.append(
                    all_tokens.unsqueeze(1)
                )  # => [b_size, 1, seq_len, hdim]
                # print(f"[Debug:Reps] Layer {L} input_rep shape={all_tokens.shape}")

            if len(layer_indices) > 0:
                input_reps = torch.cat(
                    input_rep_list, dim=1
                )  # => [b_size, n_layers, seq_len, hdim]
