...
class ExperimentRunner:
    ...
    def load_resources(self):
        ...
        # Load models
        self.self_obf_model = load_local_model(
        ...
        self.entries = create_example_variations(
            self.test_data_df, words=["bomb", "gun", "elephant"]
        )
    ...
    def run_baseline_comparisons(self):
        ...
        for entry_name, entry in self.entries.items():
            base_score = (
                get_probe_score(self.word_to_probe["gun"], entry, model=self.base_model)
                .mean()
                .item()
            )
            obf_score = (
                get_probe_score(
                    self.word_to_probe["gun"], entry, model=self.self_obf_model
                )
                .mean()
                .item()
            )
        ...
        for i in range(max_layer + 1):
            ...
            adv_score = (
                get_probe_score(
                    ...
                    gun_adv_gun_examples,
                    model=self.self_obf_model,
                )
                .mean()
                .item()
            )
    ...
    def run_layer_sweep_experiment(self):
        ...
        for num_layers in [1, 3, 5]:
            ...
            layer_sweep_results[f"{num_layers}_layer_from_obf_vanilla_gun"] = (
                layer_sweep(
                    self.base_model,
                    self.self_obf_model,
                    self.word_to_probe["gun"],
                    self.entries["Gun vanilla"],
                    num_layers,
                    max_layer,
                )
            )

            layer_sweep_results[f"{num_layers}_layer_from_base_vanilla_gun"] = (
                layer_sweep(
                    self.self_obf_model,
                    self.base_model,
                    self.word_to_probe["gun"],
    ...
    def run_representation_analysis(self):
        ...
        ) = get_dist_between_reps(
            self.entries["Gun adv gun"], self.base_model, self.self_obf_model
        )
        ...
        (
            ...
            self.entries["Gun vanilla"], self.base_model, self.self_obf_model
        )
    ...
    def run_token_probability_analysis(self, sample_idx=0):
        ...
        # Get tokens
        tokens = self.base_model.tokenizer.convert_ids_to_tokens(
            base_reps.input_ids[0].tolist()
        )
        ...
        fig_obf.write_image(
            f"{self.config.results_dir}/top_tokens_obf.png",
            width=1200,
            height=800,
            scale=2,
        )
        fig_base.write_image(
            f"{self.config.results_dir}/top_tokens_base.png",
            width=1200,
            height=800,
            scale=2,
        )
    ...
    def run_patching_experiment(self):
        ...
        nns_self_obf_model = LanguageModel(
            self.self_obf_model.model, tokenizer=tokenizer
        )
